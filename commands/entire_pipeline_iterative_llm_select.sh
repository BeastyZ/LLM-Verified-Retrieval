...
fitlog_dir=logs_no_rerank_more_details
...
k \
window_size \
reversed_browse_order \
selected_doc_first \
idf_use_letter \
use_title \
pad_with_raw_retrieval_result \
reader_temperature \
system_prompt_file_name \
used_doc_field_in_retrieval \
)
...
raw_retrieval_dir="new_data/${dataset_name}_eval_${retriever}_top100_with_summaries"
...
#  exit
  ...
  llm_select_output_dir="${llm_select_input_dir}_iterative_select_${retriever_llm}_${k}_${idf_use_letter}_${use_title}_${window_size}_${reversed_browse_order}_${selected_doc_first}_${system_prompt_file_name}_use_${used_doc_field_in_retrieval}_retrieval"
...
#  exit
    ...
    python llm_retrieval_related/main_iterative_select_supporting_docs.py \
      --k $k \
      ...
      --used_doc_field_in_retrieval $used_doc_field_in_retrieval \
      --openai_multi_thread $openai_multi_thread
  fi

  if [ "$pad_with_raw_retrieval_result" = "0" ];
  then
    final_retrieval_result_dir=$llm_select_output_dir
    final_retrieval_result_data=$llm_select_output_data
  elif [ "$pad_with_raw_retrieval_result" = "1" ];
  then
    llm_select_output_pad_dir="${llm_select_output_dir}_pad_raw_retrieval"
    mkdir -p $llm_select_output_pad_dir
    llm_select_output_pad_data="$llm_select_output_pad_dir/input.json"

    final_retrieval_result_dir=$llm_select_output_pad_dir
    final_retrieval_result_data=$llm_select_output_pad_data

    python tools/pad_retrieval_result.py \
      --input_f $llm_select_output_data \
      --output_f $llm_select_output_pad_data \
      --raw_retrieval_f $raw_retrieval_data \
      --k $k
  else
      echo "not support pad_with_raw_retrieval_result=$pad_with_raw_retrieval_result, so exit"
      exit
  fi
...
run_result_fp="${final_retrieval_result_dir}/${exp_tag}-${reader_llm}-shot${shot}-ndoc${ndoc}-seed${seed}-quick_test${quick_test}-temp${reader_temperature}"

if [ ! -f "${run_result_fp}.json" ]; then
  echo "*****************************"
...
python tools/fitlog_record_iterative_llm_select.py \
--fitlog_dir $fitlog_dir \
--raw_retrieval_data $raw_retrieval_data \
