...
if [ "$rerank_method" = "no" ]; then
  ...
  rerank_retrieval_result_dir="${raw_retrieval_dir}_rank_gpt_2_${retriever_llm}"
  mkdir $rerank_retrieval_result_dir -p

  if [ "$llm_rerank_from" = "rank_gpt" ];
  then
    rank_gpt_input_dir=$rerank_retrieval_result_dir

    rerank_retrieval_result_dir="${rerank_retrieval_result_dir}_rank_gpt_${retriever_llm}"
    mkdir $rerank_retrieval_result_dir -p

  elif [ "$llm_rerank_from" = "raw_retriever" ];
  then
    rank_gpt_input_dir="$raw_retrieval_dir"
  else
    echo "not support llm_rerank_from=$llm_rerank_from, so exit"
    exit
  fi

  rerank_retrieval_result_data="${rerank_retrieval_result_dir}/input.json"
  rank_gpt_input_data="$rank_gpt_input_dir/input.json"

  if [ -e $rerank_retrieval_result_data ]; then
    echo "rank_gpt result exists, so skip running rank_gpt"
  else
    python rerank.py \
  --data_file $rank_gpt_input_data \
