...
def compute_autoais(data,
    ...
    citation_recall = np.mean(ais_scores)
    citation_precision = np.mean(ais_scores_prec)
    return {
        "citation_rec": 100 * citation_recall,
        "citation_prec": 100 * citation_precision,
        "citation_f1": 100 * calculate_f1(citation_precision, citation_recall)
    }
...
def main():
    ...
    parser.add_argument('--eval_metric',required=True,choices=['default','correctness','custom'])
    ...
    parser.add_argument('--full_fitlog_hyper',default=0)
    ...
    if args.eval_metric == 'default':
        if 'asqa' in args.f:
            ...
            args.claims_nli = 1
            args.mauve = 1
    elif args.eval_metric == 'custom':
        pass
    elif args.eval_metric == 'correctness':
        if 'asqa' in args.f:
            args.qa = 0
            args.mauve = 0
            args.citations = 0
