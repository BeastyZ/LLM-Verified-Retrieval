...
def make_demo(item, prompt, ndoc=None, doc_prompt=None, instruction=None, use_shorter=None, test=False, use_sub_questions: int=0):
    # For demo prompt
    ...
    # Use sub questions for asqa.
    if use_sub_questions and 'qa_pairs' in item:
        questions = list(map(lambda x: x['question'], list(item['qa_pairs'])))
    else:
        questions = [item['question']]
    prompt = prompt.replace("{INST}", instruction).replace("{Q}", '\n'.join(questions))
    if "{D}" in prompt:
        if ndoc == 0:
...
def get_demonstration(demo_data) -> str:
    """
    Args
    ----
    demo_data: Dict
...
def get_messages(questions: str, prompt_style: str, doc):
    """
    Args
    ----
    questions: str
